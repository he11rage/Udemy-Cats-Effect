...
object ErrorHandlingAppExercise extends IOApp {
  ...
  object Service {
    ...
    def loadFile(filename: String): IO[Either[DomainError, String]] = {
      ...
      /* 1 */
      // Реализуйте функцию загрузки файла, которая загружает все содержимое файла в строку.
      // Если файл не существует, зафиксируйте его с ошибкой домена TextFileNotFound

      loadFileContents(filename).map {
        bytes =>
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    (validateFileName("ErrorHandling.txt") match {
      case Valid(filename) =>
